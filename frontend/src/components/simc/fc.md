...
function Simc() {
  ...
  // Generate SimC input with combinations
  const generateSimcWithCombinations = () => {
    if (!characterData || !combinations.length) return simcInput;

    let baseSimcInput = simcInput;
    
    // Extract name from the base character section
    const nameMatch = /^([^=]*)=/.exec(baseSimcInput);
    const characterName = nameMatch ? nameMatch[1].trim() : "Character";

    // Add combinations as copies
    let combinationsInput = '';
    
    combinations.forEach((combo, index) => {
      const comboNumber = index + 1;
      combinationsInput += `\ncopy="Combo ${comboNumber},${characterName}"\n`;
      combinationsInput += `### Combo ${comboNumber}\n`;
      
      // Add each item in the combination
      Object.keys(combo).forEach(slotKey => {
        const item = combo[slotKey];
        combinationsInput += `# ${item.name || slotKey} ${item.item_level || ''}\n`;
        combinationsInput += `${slotKey}=,id=${formatItemForSimc(item)}\n`;
      });

      // Copy talents from base profile
      const talentMatch = /talents=([^\n]*)/.exec(baseSimcInput);
      if (talentMatch) {
        combinationsInput += `talents=${talentMatch[1]}\n`;
      }
    });

    // Combine base input with combinations
    return baseSimcInput + combinationsInput;
  };
  ...
  const runSimulation = async () => {
    ...
    console.log(generateSimcWithCombinations());
    try {
      let input;
      if (inputMode === 'addon') {
        // Use the enhanced SimC input with combinations if available
        input = combinations.length > 0 ? generateSimcWithCombinations() : simcInput;
      } else {
        // Format armory input
